...
quarto_profiles <- c("html_lms", "docx")
...
# make all combinations of lessons and profiles
to_render <- tidyr::crossing(lessons_to_render, quarto_profiles)

# render single lesson, single profile
render_with_profile("lesson01/lesson01.qmd", profile = "docx")
...
purrr::walk(lessons_to_render, \(x) render_with_profile(x, profile = "html_book"))
